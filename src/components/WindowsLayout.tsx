...
import { ArrowLeft, Key, Minus, Power, X } from 'lucide-preact';
import { useWindowWidth } from '../utilities/function';
...
export const WindowsLayout = ({ children }: { children: any }) => {
  ...
  const smallScreen = useWindowWidth() < 640
    ...
    <div className="h-screen w-screen flex flex-col bg-[#F4F7FA] overflow-hidden text-slate-900 font-sans">
      ...
      <header className="bg-slate-900 text-white h-14 flex items-center justify-between px-4 shrink-0 shadow-lg z-20">
        <div className="flex items-center gap-3">
          ...
          {smallScreen ?
            <div className="bg-white text-black h-8 w-8 rounded-lg flex items-center justify-center font-bold text-sm shadow-lg shadow-blue-600/30">IO</div>
            :
            <div className="bg-white text-black rounded-lg flex items-center justify-center font-bold text-sm shadow-lg shadow-blue-600/30 "><img src={iotelligenceLogo} className="h-9 rounded-md" alt="" /></div>
          }
        </div>
        ...
        <div className="flex items-center gap-1 md:gap-4">
          <button className="flex flex-col items-center hover:bg-slate-800 px-3 py-1 max-sm:px-1 rounded-md transition-colors group">
            <Key size={smallScreen ? 12 : 16} className="group-hover:text-blue-400" />
            {smallScreen ? null : <span className="text-[9px] mt-0.5 text-slate-300">Change Password</span>}
          </button>

          <button className="flex flex-col items-center hover:bg-slate-800 px-3 py-1 rounded-md transition-colors group">
            <Minus size={smallScreen ? 12 : 16} className="group-hover:text-blue-400" />
            {smallScreen ? null : <span className="text-[9px] mt-0.5 text-slate-300">Minimize</span>}
          </button>

          <button className="flex flex-col items-center hover:bg-red-600 px-3 py-1 rounded-md transition-colors group">
            <Power size={smallScreen ? 12 : 16} />
            {smallScreen ? null : <span className="text-[9px] mt-0.5 text-slate-300">Stop</span>}
          </button>
        </div>
      </header>
      ...
      <nav className="bg-white border-b border-slate-200 flex px-6 py-2 gap-2 shrink-0">
        {navItems.map((item) => (
          <Link
            key={item.path}
            to={item.path}
            className={`px-6 py-1.5 rounded-full font-bold transition-all text-md max-sm:text-xs ${location.pathname.startsWith(item.path)
